...
public class CrossFormComponent extends VerticalLayout{
	...
	comboBoxGeneType.addValueChangeListener(new ValueChangeListener() {
		...
		String filePath = crossFormHelper.BASE_PATH +"/WEB-INF/files/input/"+valueString+"list.csv";
	...
	uploadEstimateSource = new Upload(null, new Upload.Receiver() {
		...
		try {
		    ...
		    filePath = filePath.replace(".csv",crossFormHelper.getTimeStamp()+".csv");
		    uploadedEstimatesFile= new File(filePath);
		    //		          tempFile = File.createTempFile("temp", ".csv");
